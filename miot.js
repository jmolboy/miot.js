...
(function () {
    ...
    //    return this.replace(/&quot;/, '\"').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/, '&');
    //};
    ...
    M.extend(M, {
	    ...
	    lang:function(key,obj){
		    ...
		    if(lang&&obj){
				for(var o in obj)
				{
					var val=obj[o];
					var preg="{"+o+"}";
					lang=lang.replace(new RegExp(preg,'g'),val);
				}
		    }
		    return lang;
	    },
        ...
        numberTrim:function(num){
            ...
            var re=/\d+?\.[0-9]+?0+$/;
            if (re.test(num)) {
                return num.replace('^(-?\d+\.[0-9]+?)0+$','$1');
            }
            return num;
        },
        objectTrim:function(obj){
            if (typeof (obj) == typeof (0)){
                return 0;
            }
            if (typeof (obj) == typeof ('') || obj instanceof String) {
                if(!isNaN(obj)&&!M.isEmpty(obj)){
                    obj=parseFloat(obj);
                    return M.numberTrim(obj);
                }
                return obj;
            }
            if (obj instanceof Array) {
                ...
                for (var i = 0; i < obj.length; i++) {
                    obj[i]= M.objectTrim(obj[i]);
                }
                return obj;
            }
            if (typeof (obj) == typeof ({})) {
                for (var p in obj) {
                    obj[p]= M.objectTrim(obj[p]);
                }
                return obj;
            }
        },
        ...
        _getjson: function (url, data, rescallback,methodtype,showloading,checkreq) {
            var reqUrl = url;
            /**/
            if (url.indexOf("?") > -1) {
                //reqUrl = url + "&" + M.jsonToQueryStr(data)+"&t=" + new Date().getMilliseconds();
			...
			{
				var loadingbox=$('.graylayer[tag=loading]');
				if(loadingbox.length>0){
					loadingbox.show();
				}
				else{
					$('<div class="graylayer" tag="loading" style="z-index:1060"><i class="loading-t"></i></div>').appendTo($(document.body));
				}
        	}
			if(M.isEmpty(checkreq)){
				checkreq=1;
			}
            var mtype=M.isEmpty(methodtype)?"post":methodtype;
           if(!M.checksendstatus(url,data)&&checkreq==1){
        	   return ;
           }
            if(!M.isEmpty(M.useragent)&&M.useragent=='inApp'){
                data['cookiekey']= M.cookeikey;
            }
            $.ajax({
                type: mtype,
                data:data,
                ...
                success: function (e) {
                	if(showloading&&!M.isEmpty(showloading))
        			{
        				var loadingbox=$('.graylayer[tag=loading]').hide();
        			}
                    var res = null;
                    if (e == undefined || e == '') {
                        res = { status: "NODATA" };
                    ...
                    {
                        if(!M.isEmpty(M.useragent)&&M.useragent=='inApp'){
                            location.href="daxidi://login.daxidi.com/share/login";
                            return;
                        }
                        if(undefined !=loginpage && !M.isEmpty(loginpage)) {
                            loginpage.logintag_click();
                            return;
                        }
                        else{
                            if(!M.isEmpty(res.loginurl)){
                                $.cookie('fromurl',location.href,{'expires':20*3600,'path':'/'});
                                location.href= res.loginurl;
                                return;
                            }
                        }
                    }
                    // res= M.objectTrim(res);
                    if (rescallback != null) {
                        rescallback.call(rescallback.context, res);
                    }
                    M.clearsendstatus(url,data);
                },
                error: function () {
                	if(showloading&&!M.isEmpty(showloading))
        			{
        				var loadingbox=$('.graylayer[tag=loading]').hide().remove();
        			}
                    if (rescallback != null) {
        ...
        checksendstatus:function(url,data){
        	var key=url;
        	for(i in data){
        		key+=data[i];
        	}
        	
        	var request=this.requestlist[key];
        	var timenow=M.getTime();
        	if(!M.isEmpty(request)&&request.sendstatus==1){
        		var sendtime=request.sendtime;    		
        		var timeDiff = (((timenow-sendtime)%(24*3600*1000))%(60*1000))/1000;
        		if(timeDiff>2){
        			this.clearsendstatus(url, data);
        			return true;
        		}
        		return false;
        	}else{
        		this.requestlist[key]={'sendstatus':1,"sendtime":timenow};
        		return true;
        	}
        },
        clearsendstatus:function(url,data){
        	var key=url;
        	for(i in data){
        		key+=data[i];
        	}
        	var sendlist={};
        	for(k in this.requestlist){
        		if(k!=key){
        			sendlist[k]=this.requestlist[k];
        		}
        	}
        	this.requestlist=sendlist;
        },
    ...
    M.browser = {
        ...
        mobile: !!J.match(/applewebkit.*mobile.*/)
    };
...
M.extend(M.Controls.Popup.prototype,
{
    ...
    {
        var autohide=this.classes.autoclose;
        if(autohide){
            M.ClosePopup();
        }
        /*this.close();*/
...
{   
    ...
    onclick:function(e){
        ...
        if(t=="dropdownlist"){
        	$('body').find(".droplist_on").removeClass("droplist_on").children("div").hide();
    ...
    {
    	var ele = M.EventEle(e);
    	if(ele.parents("div[tag=dropdownlist]").length==0){
    		 this.listdom.removeClass("droplist_on");
    	     this._hide();
    	}
    },
...
M.extend(M, {
    ...
    closeMessage:function(){
        var ui_messageBox = $(document.body).children("#ui_message");
        var transparentlayer = $(document.body).children("#lightgraylayer");
...
M.extend(M.Controls.Ga.prototype,
{
    ...
    ga:function(ele){
        ...
        var cate='';
        if(gname.indexOf('order-')>=0){
            var formtpl=ele.parents("div[tag=popform]");
            var action=$("#ordercell").attr("action");
            if(action=='add'){
                gname=gname.replace('order-','预定-');
                cate='预定';
            }else if(action=='edit'){
                gname=gname.replace('order-','修改-');
                cate='修改';
            }else{
                gname=gname.replace('order-','补录-');
                cate='补录';
            }
        }else{
            if(gname.indexOf('-')>=0){
                var cate_arr=gname.split('-');
                cate=cate_arr[0];
            }else{
                cate='none';
            }
